...
class LassoTraining(BaseModelLinear):
    def __init__(
        ...
        self.model_lasso = LassoCV(
            fit_intercept=False, alphas=[0, 0.1, 0.5, 1, 2, 10, 100], cv=5
        )
        self.Xtrain_feature_selection, self.Xtest_feature_selection = (
            self.precessing_with_model()
        )

    # Lasso auto shrink the parameter so we don't need to feature selection
    ...
    def training(self, train_force=True):
        Xtrain_feature_selection, _ = self.precessing_with_model()
        Xtrain, ytrain = Xtrain_feature_selection, self.ytrain
        t1 = time.time()
        self.model_lasso.fit(X=Xtrain, y=ytrain)
        self.time = time.time() - t1
        self.coef_ = self.model_lasso.coef_
        with open(self.model_path, "wb+") as f:
            pkl.dump(self.model_lasso, f)
        return self.model_lasso

    def eval_model(self, force_train=True):
        if os.path.exists(self.model_path) and not force_train:
            model_lasso = self.load_model()
        else:
            model_lasso = self.training()
        self.ypred = model_lasso.predict(self.Xtest_feature_selection)
...
class LassoPredict(BasePredict):
    ...
    def load_model(self):
        model_path = self.model_info.get("model_path").get("lasso")
